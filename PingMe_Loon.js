//2026/05/14 Loon 适配版，基于原版 PingMe.js 改写
/*
...
// 一次性执行全部账号；保留账号间隔，避免请求过快。
const RUN_ALL_ACCOUNTS = true;
...
function manageAccountsIfNeeded(store) {
  ...
  if (action === 'list') {
    notify('📋 PingMe 账号列表', accountListText(store));
    return true;
  }
  ...
  if (/^del:\d+$/.test(action)) {
    ...
    saveStore(store);
    notify('✅ 删除成功', `🗑️ 已删除第 ${idx} 个账号：${alias}\n📋 剩余账号：\n${accountListText(store)}\n⚙️ 请清空 action 参数`);
    return true;
  }
  ...
  if (/^rename:\d+:.+/.test(action)) {
    ...
    saveStore(store);
    notify('✅ 备注成功', `🏷️ 第 ${idx} 个账号已改为：${name}\n📋 当前账号：\n${accountListText(store)}\n⚙️ 请清空 action 参数`);
    return true;
  }
...
function httpGet(url, headers) {
  return new Promise((resolve, reject) => {
    $httpClient.get({ url, headers }, (error, response, data) => {
      ...
      resolve({ statusCode: response && response.status, headers: response && response.headers, body: data });
    });
  });
}
