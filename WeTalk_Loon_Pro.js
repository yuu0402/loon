...
function buildUA(baseUA, seed) {
  ...
  return `WeTalk/1.0.0 (${model}; iOS ${iosVer}; Scale/${scale}) CFNetwork/${cfn} Darwin/${darwin}`;
}
...
function buildHeaders(capture, ua) {
  const headers = normalizeHeaderNameMap(capture.headers || {});
...
function httpGet(url, headers) {
  return new Promise((resolve, reject) => {
    ...
    $httpClient.get({ url, headers }, (error, response, data) => {
      ...
      log('HTTP BODY', typeof data === 'string' ? data.slice(0, 500) : data);
...
async function runAccount(acc, index, total) {
  ...
  try {
    ...
    try {
      ...
      log('FINAL BALANCE PARSE ERROR', String(e));
    }
  } catch (e) {
...
function handleCapture() {
  ...
  const baseUA = headersMap['user-agent'] || '';
...
async function handleCron() {
  ...
  log('ACCOUNT IDS', ids);
  if (!ids.length) {
    notify('⚠️ 未抓到任何账号', '请先开启“WeTalk获取参数”并打开 App 触发一次抓包');
    return $done();
  }
